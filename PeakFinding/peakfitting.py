...
def fit1peak(inim, ipkxy=np.array([]), ithresh=.7, calcCVHullMask=True, calcprediction=True, calcresidual=True, minHW=np.array([1,1], dtype=np.int8)):
    ...
    bbx, msk = bboxthresh(inim,ptxy=ipkxy,thr=ithresh,convexhullmask=calcCVHullMask,normalize=True,minHW=minHW)
    ind=np.where(msk==1)
...
def refinePeaks(inim, ipkxy, winsz=[], ithresh=.5, minHW=np.array([1,1],dtype=np.int8), progressDescr='Peak Fitting...', verbose=0, **kwargs):
    #### Inputs ###
    #inim           :           input image
    ...
    for i in tqdm(range(ipkxy.shape[0]), progressDescr, disable=(verbose==0)):
        try:
            if winsz.size==2:
                winsz_lp = winsz
            else:
