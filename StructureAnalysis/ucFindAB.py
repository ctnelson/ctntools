...
def abGuessFromScoredPeaks(pks, xy0=np.zeros((2,)), alphaGuess=90, alphaExclusions=[[0,10],[190,10]], rexcl=0, rGuess=None, aOrientTarget=0, awt = [.75,0.05,.2], bwt=[1/3,1/3,1/3], normalize=1, **kwargs):
    ...
    #angdelta = angdelta/np.max(np.abs(angdelta))
    angdelta = np.abs(distWrapped(pks[:,4],pks[aind,4]-alphaGuess)) / np.pi
...
def ucFindAB(im, imMask=None, ucScaleGuess=None, swUCScalar=4.1, pxlprUC=20, downsampleFlag=True, rExclScalar=1, rExclSource='firstMin', alphaGuess=None, inax=[None]*3, verbose=0, **kwargs):
    ...
    outDict = {'swIm':swid, 'pks':pks, 'scores':scores, 'aInd':aInd, 'bInd':bInd, 'dists':dists, 'radDistr':distr, 'dInd':distInds, 'center':xy0}
    return a, b, outDict
